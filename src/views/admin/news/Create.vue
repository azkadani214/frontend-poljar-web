  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div>
          <h2 class="text-xl font-bold text-gray-900">Tulis Berita Baru</h2>
          <p class="text-xs text-gray-500">
            Sampaikan informasi terbaru kepada publik
          ...
          <CardContent class="space-y-6">
            <div class="space-y-2">
              ...
              <Button
                variant="link"
                size="sm"
                class="px-0 h-auto text-primary-orange"
                @click="router.push('/admin/news/categories')"
              >
      ...
      <AlertDialogContent class="max-w-[400px] p-0 overflow-hidden border-none shadow-2xl">
        <div class="bg-gradient-to-br from-primary-navy to-blue-900 p-6 text-white relative">
          ...
          <div class="relative z-10 flex flex-col items-center text-center space-y-4">
            ...
            <div>
              <h3 class="text-xl font-bold">Pulihkan Draft Berita?</h3>
              <p class="text-white/70 text-sm mt-1">
                Kami menemukan draft berita yang belum selesai dari sesi sebelumnya.
              </p>
            </div>
          </div>
        </div>
...
import { ArrowLeft, Send, Loader2, X, Plus, RotateCcw, Newspaper } from 'lucide-vue-next'
...
const DRAFT_KEY = computed(() => `news_draft_${authStore.user?.id || 'guest'}`)
...
async function handleSubmit() {
  ...
  try {
    ...
    // Explicitly append required fields to avoid any loop confusion
    ...
    // Categories (backend expects array categories[])
    if (form.category_id) {
...
function confirmRestore() {
  if (pendingDraft.value) {
    ...
    toast.info('Draft berita berhasil dipulihkan')
