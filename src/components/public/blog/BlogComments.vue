<template>
  <div class="mt-16 pt-16 border-t-2 border-gray-100">
    ...
    <div v-else-if="comments.length" class="space-y-12">
      <div v-for="comment in comments" :key="comment.id" class="flex gap-6 group">
        ...
        <div class="flex-grow">
          ...
          <button
            class="mt-4 text-xs font-bold text-primary-orange hover:text-primary-navy transition-colors tracking-widest uppercase flex items-center gap-2"
          >
            <Reply class="w-3 h-3" />
            Balas
          </button>
...
import { ArrowRight, MessageSquare, Reply } from 'lucide-vue-next'
...
const fetchComments = async () => {
  loading.value = true
  try {
    const response = await blogStore.fetchComments(props.postSlug)
...
const handleSubmit = async () => {
  ...
  try {
    const response = await blogStore.submitComment(props.postSlug, form.value)
